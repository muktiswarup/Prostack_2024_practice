...
// Dishes data for Little Lemon restaurant
const dishData = [
    {
        name: "Italian pasta",
        price: 9.55
...
// Step 1 to Step 7: The getPrices() function
function getPrices(taxBoolean) {
    for (let dish of dishData) {
        let finalPrice;  // Step 3: Declare a finalPrice variable
        
        if (taxBoolean === true) {  // Step 4: Check if taxBoolean is true
            finalPrice = dish.price * tax;  // Calculate final price with tax
        } else if (taxBoolean === false) {  // Step 5: Check if taxBoolean is false
            finalPrice = dish.price;  // Assign original price
        } else {  // Step 6: Handle non-boolean values
...
// Step 8 to Step 12: The getDiscount() function
function getDiscount(taxBoolean, guests) {
    // Step 9: Call getPrices() within getDiscount()
    getPrices(taxBoolean);

    // Step 10: Defensive coding for the guests argument
    if (typeof guests === 'number' && guests > 0 && guests < 30) {
        let discount = 0;  // Step 11: Declare discount variable
        
        // Apply discount based on number of guests
        if (guests < 5) {
            discount = 5;  // Assign 5 if guests are less than 5
        } else if (guests >= 5) {
            discount = 10;  // Assign 10 if guests are 5 or more
        }
        
        console.log('Discount is: $' + discount);  // Log discount
    } else {
        // Step 12: Handle invalid guest input
        console.log('The second argument must be a number between 0 and 30');
    }
}

// Examples to test the functions
getDiscount(true, 2);   // This should apply tax and give a discount of $5
getDiscount(false, 10); // This should not apply tax and give a discount of $10

// Edge cases
getDiscount();          // No arguments passed
getDiscount(true, 50);  // Invalid number of guests
getDiscount(true, "five");  // Invalid type for guests
